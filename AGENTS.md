...
**Last agent action:** Duplicate persona + character cards from list row menus (new id, “(copy)” name, avatar + embedded lorebook); prior: API browse filters + persisted picker + chat quick API menu.
...
- **Settings hub** — separate menus:
 - **Personas** — create multiple {{user}} identities; **list may be empty**; **~token badges** on list + live count in editor; **Compact persona…** (editor menu, review before apply); **pick persona when starting solo or group chat** (defaults to app default or **Plain User**); per-chat switch in ⋮ menu; **Duplicate** from row menu; **AI persona builder**; **AI wand**; **Generate avatar**; optional default for new chats
 - **Characters** — character cards + **categories**; **list may be empty** (no forced starter card); **temporary characters**; **Full cards only** filter; filter; **consistency check + one-tap fix**; AI card builder; **section chips**; avatar + consistency in ⋮ menu; **Duplicate** from row menu (copies lorebook + categories); **~token badges**
